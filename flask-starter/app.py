...
def profile(username):
    """Displays the profile page for the given username."""
    uid = session.get("uid")
    if not uid:
    ...
    try:
        ...
        currents_result, reviews_result = f.profile_render(conn, session)
        return render_template(
            "profile.html",
            page_title="Profile",
            username=username,
            currentsResult=currents_result,
            reviewsResult=reviews_result,
            user_id=uid,
        )
